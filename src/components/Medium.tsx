...
async function fetchMediumRows(): Promise<MediumRow[]> {
  try {
    const res = await fetch('/r/medium.json', { cache: 'no-store' })
    if (!res.ok) return []
    const data = (await res.json()) as MediumRow[]
...
export function Medium() {
  const [rows, setRows] = React.useState<MediumRow[] | null>(null)

  React.useEffect(() => {
    let cancelled = false
    fetchMediumRows().then(r => {
          ...
          >
            <div className="space-y-2 flex-1 flex flex-col">
              ...
              <p className="text-sm text-muted-foreground leading-snug">
                Curated Medium author for detailed security write-ups.
              </p>
