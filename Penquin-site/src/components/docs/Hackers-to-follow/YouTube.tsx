...
async function fetchYouTubeRows(): Promise<YouTubeRow[]> {
  try {
    const res = await fetch('/r/youtube.json', { cache: 'no-store' })
    if (!res.ok) return []
    const data = (await res.json()) as YouTubeRow[]
    return data.filter(r => typeof r.username === 'string' && typeof r.href === 'string')
  } catch {
    return []
  }
}
...
export function YouTube() {
  const [rows, setRows] = React.useState<YouTubeRow[] | null>(null)

  React.useEffect(() => {
    let cancelled = false
    fetchYouTubeRows().then(r => {
          ...
          >
            <div className="space-y-2 flex-1 flex flex-col">
              ...
              <p className="text-sm text-muted-foreground leading-snug">
                Curated channel for security, engineering, and programming content.
              </p>
